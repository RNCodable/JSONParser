// Last sync: https://github.com/apple/swift-corelibs-foundation/blob/9c4cec91fe344dd25c23aa1971eb36f926bfcaf4/Tests/Foundation/Tests/TestJSONEncoder.swift
...
class TestRNJSONEncoder : XCTestCase {
    ...
    func test_encodingTopLevelFragments() {
        ...
        func _testFragment<T: Codable & Equatable>(value: T, fragment: String) {
            ...
            do {
                data = try RNJSONEncoder().encode(value)
            ...
            do {
                let decodedValue = try RNJSONDecoder().decode(T.self, from: data)
...
""")
    ...
    func test_encodingNonConformingFloatStrings() {
        let encodingStrategy: RNJSONEncoder.NonConformingFloatEncodingStrategy = .convertToString(positiveInfinity: "INF", negativeInfinity: "-INF", nan: "NaN")
        let decodingStrategy: RNJSONDecoder.NonConformingFloatDecodingStrategy = .convertFromString(positiveInfinity: "INF", negativeInfinity: "-INF", nan: "NaN")
    ...
    func test_codingOfBool() {
        ...
        do {
            _ = try RNJSONDecoder().decode([Bool].self, from: "[1]".data(using: .utf8)!)
        ...
        func testValue(_ valueName: String) {
            do {
                ...
                _ = try RNJSONDecoder().decode(Foo.self, from: jsonData)
        ...
        if let falseFoo = try? RNJSONDecoder().decode(Foo.self, from: falseJsonData) {
        ...
        if let trueFoo = try? RNJSONDecoder().decode(Foo.self, from: trueJsonData) {
    ...
    func test_codingOfFloat() {
        ...
        XCTAssertThrowsError(try RNJSONDecoder().decode(Float.self, from: "1e100".data(using: .utf8)!))
    }
    ...
    func test_codingOfDouble() {
        ...
        // Check value too large fails to decode.
        XCTAssertThrowsError(try RNJSONDecoder().decode(Double.self, from: "100e323".data(using: .utf8)!))
    }
    ...
    func test_codingOfDecimal() {
        ...
        // Check value too large fails to decode.
        XCTAssertThrowsError(try RNJSONDecoder().decode(Decimal.self, from: "100e200".data(using: .utf8)!))
    }
    ...
    func test_encodeDecodeNumericTypesBaseline() throws {
        ...
        let data = try RNJSONEncoder().encode(source)
        let destination = try RNJSONDecoder().decode(NumericTypesStruct.self, from: data)
        ...
        let destination2 = try RNJSONDecoder().decode(NumericTypesStruct.self, from: Data(json.utf8))
    ...
    func test_numericLimits() {
        ...
        func decode(_ type: String, _ value: String) throws {
            ...
            key.append("Value")
            _ = try RNJSONDecoder().decode(DataStruct.self, from: "{ \"\(key)\": \(value) }".data(using: .utf8)!)
        }
    ...
    func test_OutputFormattingValues() {
        XCTAssertEqual(RNJSONEncoder.OutputFormatting.prettyPrinted.rawValue, 1)
        if #available(OSX 10.13, *) {
            XCTAssertEqual(RNJSONEncoder.OutputFormatting.sortedKeys.rawValue, 2)
        }
        XCTAssertEqual(RNJSONEncoder.OutputFormatting.withoutEscapingSlashes.rawValue, 8)
    }
    ...
    func test_SR17581_codingEmptyDictionaryWithNonstringKeyDoesRoundtrip() throws {
        ...
        let data = try RNJSONEncoder().encode(toEncode)
        let result = try RNJSONDecoder().decode(Something.self, from: data)
    ...
    private func _testEncodeFailure<T : Encodable>(of value: T) {
        do {
            let _ = try RNJSONEncoder().encode(value)
    ...
    private func _testRoundTrip<T>(of value: T,
                                   ...
                                   outputFormatting: RNJSONEncoder.OutputFormatting = [],
                                   dateEncodingStrategy: RNJSONEncoder.DateEncodingStrategy = .deferredToDate,
                                   dateDecodingStrategy: RNJSONDecoder.DateDecodingStrategy = .deferredToDate,
                                   dataEncodingStrategy: RNJSONEncoder.DataEncodingStrategy = .base64,
                                   dataDecodingStrategy: RNJSONDecoder.DataDecodingStrategy = .base64,
                                   nonConformingFloatEncodingStrategy: RNJSONEncoder.NonConformingFloatEncodingStrategy = .throw,
                                   nonConformingFloatDecodingStrategy: RNJSONDecoder.NonConformingFloatDecodingStrategy = .throw) where T : Codable, T : Equatable {
...
extension TestRNJSONEncoder {
    static var allTests: [(String, (TestRNJSONEncoder) -> () throws -> Void)] {
        return [
